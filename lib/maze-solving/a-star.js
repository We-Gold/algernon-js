...
export const solveAStar = (
	...
	const cameFrom = [...Array(rawMaze.length)].map((_) =>
		Array(rawMaze[0].length)
	)

	// Stores the cost of the cheapest path from start
	// to a given node (default Infinity)
	const gScores = [...Array(rawMaze.length)].map((_) =>
		Array(rawMaze[0].length).fill(Infinity)
	)
	...
	// (default Infinity)
	const fScores = [...Array(rawMaze.length)].map((_) =>
		Array(rawMaze[0].length).fill(Infinity)
	)
