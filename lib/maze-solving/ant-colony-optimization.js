	...
	for (let iteration = 0; iteration < config.maxIterations; iteration++) {
		for (let antIndex = 0; antIndex < ants.length; antIndex++) {
			...
			if (antRow === goal[0] && antCol === goal[1]) {
				...
				return reconstructPath(visitedStacks[antIndex])
			}
...
const reconstructPath = (visitedStack) => {
	const path = []

    // Add all of the points in the path in reverse order
	for (const index of visitedStack) 
		path.push(index)
