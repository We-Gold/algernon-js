 ...
 * @param {number[][]} cells
 */
export const removeWallGrid = (
	[currentRow, currentCol],
	[neighborRow, neighborCol],
	cells
) => {
	const [midRow, midCol] = [
		currentRow + (neighborRow - currentRow) / 2,
		currentCol + (neighborCol - currentCol) / 2,
	]

	// Remove the wall between the cells
	cells[midRow][midCol] = false
...
/**
 * Selects a random index within a range,
 * but only even indices can be selected
 * @param {number} length
 * @returns {number} A random index
 */
export const chooseRandomIndexGrid = (length) => {
 ...
 * @returns {number[][]} All unvisited neighbors of the cell
 */
export const getUnvisitedNeighborsGrid = ([row, col], cells) => {
	const neighbors = [
		[row - 2, col], // North
		[row + 2, col], // South
		[row, col + 2], // East
	...
	// Select cells that are within bounds and aren't visited
	return neighbors.filter((neighbor) => cells?.[neighbor[0]]?.[neighbor[1]])
}
...
export const getAvailableNeighbors = ([row, col], cells) => {
	...
	// Select cells that are within bounds and aren't blocked
 ...
 * @returns {number[][]} All available neighbors of the current cell
 */
export const getAvailableNeighborsGrid = ([row, col], cells) => {
	const neighbors = [
		[row - 1, col], // North
		[row + 1, col], // South
		[row, col + 1], // East
...
/**
 * @param {number} rows The number of rows the 2D array will have
 * @param {number} cols The number of columns the 2D array will have
 * @param {any} value The value to fill each cell with
 * @returns {any[][]}
 */
export const createFilledMatrix = (rows, cols, value) =>
	[...Array(rows)].map((_) => Array(cols).fill(value))
