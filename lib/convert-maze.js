...
export const supersampleMaze = (rawMaze, factor = 2) => {
	...
	const largeMaze = [...Array(rows)].map((_) => Array(cols).fill(0))
 ...
 */
export const fillWallsWithCells = (rawMaze, wallSymbol = 1, cellSymbol = 0, supersampleFactor = 1) => {
	...
	// Create a placeholder maze to hold the new cells
	const walledMaze = [...Array(rows)].map((_) => Array(cols).fill(cellSymbol))

	// Add south and east walls, and fill in any corners
	for (let row = 0; row < rawMaze.length; row++) {
		for (let col = 0; col < rawMaze[row].length; col++) {
			const [walledRow, walledCol] = [row * cellWallSize + row, col * cellWallSize + col]
			...
			if (hasSouthWall && row < rawMaze.length - 1) {
				for (let i = 0; i < cellWallSize; i++) {
					walledMaze[walledRow + cellWallSize][walledCol + i] = wallSymbol
				}

				// Add a south wall to the the left
				if (col > 0) walledMaze[walledRow + cellWallSize][walledCol - 1] = wallSymbol

				// Fill in any missing corners
				if (hasWestWall && col > 0) {
					walledMaze[walledRow + cellWallSize][walledCol - 1] = wallSymbol
				} else if (hasEastWall) {
					walledMaze[walledRow + cellWallSize][walledCol + cellWallSize] = wallSymbol
				}
			}
			...
			if (hasEastWall && col < rawMaze[row].length - 1) {
				for (let i = 0; i < cellWallSize; i++) {
					walledMaze[walledRow + i][walledCol + cellWallSize] = wallSymbol
				}

				// Add an east wall to the north
				if (row > 0) walledMaze[walledRow - 1][walledCol + cellWallSize] = wallSymbol

				// Fill in any missing corners
				if (hasNorthWall && row > 0) {
					walledMaze[walledRow - 1][walledCol + cellWallSize] = wallSymbol
				} 
			}
		}
	}
